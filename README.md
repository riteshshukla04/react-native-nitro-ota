...
Helper function to generate download and version URLs from a GitHub repository.

**Parameters:**
- `githubUrl` (string): Full GitHub repository URL (e.g., `https://github.com/user/repo`)
- `otaVersionPath` (string, optional): Path to version file in repo, defaults to `'ota.version'`
- `ref` (string, optional): Branch, tag, or commit ref, defaults to `'main'`

**Returns:** `{ downloadUrl: string, versionUrl: string }`
...
const { downloadUrl, versionUrl } = githubOTA({
  ...
  otaVersionPath: 'ota.version',  // optional
  ref: 'main'  // optional, can be branch, tag, or SHA
});
...
// https://raw.githubusercontent.com/your-username/your-repo/main/ota.version
```
...
Convenient wrapper class for managing OTA updates with a cleaner API.
...
- `versionUrl` (string): URL to check version
...
**Returns:** `Promise<string>` - Path where bundle was extracted
...
Gets the currently installed OTA version.

**Returns:** `string | null` - Version string or null

**Example:**
```typescript
const version = otaManager.getVersion();
console.log('Current version:', version);
```

##### `getUnzippedPath()`

Gets the path to the extracted bundle.

**Returns:** `string | null` - Bundle path or null

**Example:**
```typescript
const path = otaManager.getUnzippedPath();
...
**Example:**
```typescript
otaManager.clearStoredData();
// App will use default bundle on next launch
```
...
#### `NitroOta.downloadAndUnzipFromUrl(downloadUrl, versionCheckUrl?)`

Downloads a zip file, extracts it, and stores the bundle path.

**Parameters:**
- `downloadUrl` (string): URL to download the zip bundle
- `versionCheckUrl` (string, optional): URL to fetch version information

**Returns:** `Promise<string>` - Path where bundle was extracted

**Example:**
```typescript
const path = await NitroOta.downloadAndUnzipFromUrl(
  'https://cdn.example.com/app-v1.2.3.zip',
  'https://cdn.example.com/version.txt'
);
```

---

### `NitroOta.checkForUpdates(versionCheckUrl?)`

Checks if a new version is available.

**Parameters:**
- `versionCheckUrl` (string, optional): URL to check version. Uses stored URL if not provided.
...
**Example:**
```typescript
const version = NitroOta.getStoredOtaVersion();
console.log('Current version:', version);
```

---

### `NitroOta.getStoredBundlePath()`

Gets the path to the stored bundle.

**Returns:** `string | null` - Bundle path or null

**Example:**
```typescript
const path = NitroOta.getStoredBundlePath();
console.log('Bundle path:', path);
```

---

### `NitroOta.clearStoredData()`
...
```typescript
NitroOta.clearStoredData();
// App will now use original bundle on next launch
```

## 🌐 Server Setup (Server Agnostic)

NitroOta works with **any server or CDN**. Here are some common setups:

### Option 1: AWS S3 + CloudFront

```typescript
const BUNDLE_URL = 'https://d111111abcdef8.cloudfront.net/bundles/latest.zip';
const VERSION_URL = 'https://d111111abcdef8.cloudfront.net/bundles/version.txt';
```

### Option 2: GitHub Releases

```typescript
import { getLatestReleaseUrl } from 'react-native-nitro-ota';

const { downloadUrl, versionUrl } = await getLatestReleaseUrl(
  'your-username',
  'your-repo'
);
```
...
```typescript
const BUNDLE_URL = 'https://api.yourserver.com/ota/bundle.zip';
const VERSION_URL = 'https://api.yourserver.com/ota/version';
```

### Option 4: Firebase Storage

```typescript
const BUNDLE_URL = 'https://firebasestorage.googleapis.com/v0/b/your-app/o/bundles%2Flatest.zip?alt=media';
const VERSION_URL = 'https://firebasestorage.googleapis.com/v0/b/your-app/o/version.txt?alt=media';
```
...
```
bundle.zip
├── index.android.bundle  (for Android)
├── main.jsbundle         (for iOS)
└── ota.version           (optional version file)
```
