...
export interface OTAVersionConfig {
  ...
  releaseNotes?: string;
  /**
   * Differential updates: patch zip per currently installed version, keyed by the exact
   * stored OTA version string. Values are absolute URLs or paths relative to this manifest.
   * Generated by `npx nitro-ota patch`.
   */
...
/**
 * Details of the last download performed by `OTAUpdateManager.downloadUpdate()`.
 */
export interface OTADownloadInfo {
  /** URL that was downloaded (patch zip or full zip) */
  url: string;
  /** true when a differential patch was applied instead of the full bundle */
  patch: boolean;
  /** Bytes downloaded */
  bytes: number;
}
...
export class OTAUpdateManager {
  ...
  /** Details of the most recent successful `downloadUpdate()` call, or null */
  lastDownload: OTADownloadInfo | null = null;
   ...
   *
   * When the version manifest lists a patch for the currently installed version, the
   * (much smaller) patch zip is downloaded and the bundle is rebuilt on device. Any patch
   * failure falls back to the full download, so `onProgress` may restart from zero once.
  ...
  async downloadUpdate(
    ...
    let bytes = 0;
    const trackProgress = (received: number, total: number) => {
      if (received === total) {
        bytes = received;
      }
      onProgress?.(received, total);
    };
    const patchUrl = await this.findPatchUrl();
    if (patchUrl) {
      try {
        console.log(`OTA: Downloading patch from ${patchUrl}`);
        const path = await downloadZipFromUrl(
          patchUrl,
          trackProgress,
          bundleFilePath
        );
        this.lastDownload = { url: patchUrl, patch: true, bytes };
        console.log(`OTA: Patch applied, update at: ${path}`);
        return path;
      } catch (error) {
        console.warn('OTA: Patch failed, downloading full bundle:', error);
      }
    }
    try {
      ...
      this.lastDownload = { url: this.downloadUrl, patch: false, bytes };
  ...
  private async findPatchUrl(): Promise<string | null> {
    ...
    try {
      return await findPatchUrl(this.versionCheckUrl, currentVersion);
    } catch (error) {
      console.warn('OTA: Could not read patch manifest:', error);
      return null;
    }
  }
